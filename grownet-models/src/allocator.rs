...
pub trait ArrayID {
    type D: Dimension;
    unsafe fn get<T>(&self, alloc: &ArrayAllocator<T>) -> ArrayView<T, Self::D>;
    unsafe fn get_mut<T>(&mut self, alloc: &ArrayAllocator<T>) -> ArrayViewMut<T, Self::D>;
    fn get_copy<T: Clone>(&self, alloc: &ArrayAllocator<T>) -> Array<T, Self::D> {
        unsafe { self.get(alloc).to_owned() }
    }
    fn alloc<T>(alloc: &mut ArrayAllocator<T>, dim: Self::D) -> Self;
}
...
impl<D: Dimension> ArrayID for ParamId<D> {
    ...
    unsafe fn get<T>(&self, alloc: &ArrayAllocator<T>) -> ArrayView<T, Self::D> {
        let ptr = alloc.ptr.as_ptr().add(alloc.offsets[self.idx]);
    ...
    fn alloc<T>(alloc: &mut ArrayAllocator<T>, dim: D) -> Self {
...
pub struct ModelAllocator<T> {
    staticvar: ArrayAllocator<T>,
    tempvar: ArrayAllocator<T>,
    generation: Wrapping<usize>
}

impl<T: Float + LinalgScalar> ModelAllocator<T> {
    pub fn new() -> Self {
        ModelAllocator { staticvar: ArrayAllocator::new(), tempvar: ArrayAllocator::new(), generation: Wrapping(0) }
    }
