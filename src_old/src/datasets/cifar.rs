...
fn test_cifar10_loading() {
    ...
    let mut dataset = Cifar10::build_train(path::Path::new(path), 0).unwrap();
    let k = dataset.next().unwrap();
    println!("im size {:?}", k.image.image.dim());
    let _test_data = Cifar10::build_test(path::Path::new(path), 1).unwrap();
