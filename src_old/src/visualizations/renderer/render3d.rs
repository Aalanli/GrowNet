...
impl Default for VectorFieldPlugin {
    fn default() -> Self {
        ...
        let angles = positions.iter().map(|ps| {
            let x = ps.x - 3.0;
            let y = ps.y - 3.0;
            let z = ps.z - 3.0;
...
fn setup_field(
    ...
    let max: Vec3 = field.bases.iter().fold(field.bases[0], |acc, elem| {
        acc.max(*elem)
    });
    ...
    let field_id = commands.spawn_bundle(PbrBundle {
    ...
    for i in 0..field.bases.len() {
        ...
        let mut transform = Transform::from_scale(Vec3::new(100.0 * length + 500.0, width, width))
            .with_translation((xyz - shift) * scale + Vec3::from_array([0.0, GLOBAL_SCALE / 2.0 + 0.3, 0.0]));
        ...
        commands.entity(field_id).add_child(
            vec
        );
    }
}
