...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        in_channels,                # Number of output color channels.
        out_channels,
        is_last
    ):
        super().__init__()
        ...
        self.synthesis_blocks = torch.nn.ParameterList()
        in_res *= 2
        while in_res < out_res:
            self.synthesis_blocks.append(SynthesisBlock(
                in_channels,
                in_channels // 2,
                w_dim=w_dim,
                resolution=in_res,
        ...
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=0, w_dim=w_dim, num_ws=None, w_avg_beta=None)

    def forward(self, z, x, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        ws = self.mapping(z, None, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
        for b in self.synthesis_blocks:
            x = b(x, ws, **synthesis_kwargs)
        return x

if __name__ == '__main__':
    model = Generator(512, 512, 8, 32, 32, 8, True)
    z = torch.randn([4, 512])
