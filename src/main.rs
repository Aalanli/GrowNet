...
mod renderer;
use renderer::meshes::build_circle;
...
fn setup(
    ...
    mut test_material: ResMut<Assets<TestMaterial>>,
) {
    let material = test_material.add(TestMaterial {} );
    // plane
    ...
    //commands.spawn_bundle(PbrBundle {
    //    mesh: meshes.add(build_circle(2.0, 10)),
    //    material: materials.add(Color::rgb(0.3, 0.4, 0.5).into()),
    //    transform: Transform::from_xyz(1.0, 0.2, 0.4).with_rotation(Quat::from_rotation_y(0.3)).with_rotation(Quat::from_rotation_x(-0.5)),
    //    ..default()
    //});
